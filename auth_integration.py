...
class LoginWindow:
    """Login window for HelloToolbelt"""
    ...
    def _create_widgets(self):
        ...
        form_frame = tk.Frame(main_frame, bg="#2b2b2b")
    ...
    def _handle_login_result(self, success, result):
        if success:
            ...
            # Show loading state before closing
            self.login_btn.config(text="Loading HelloToolbelt...")
            self.error_label.config(text="", fg="#4ade80")
            self.window.update()
            ...
            # Destroy window first, then call success callback
            self.window.destroy()
        else:
    ...
    def run(self):
        ...
        # After window closes, call success callback if login was successful
        if self.result and hasattr(self, '_success_callback') and hasattr(self, '_auth_result'):
            self._success_callback(self._auth_result)
        return self.result
